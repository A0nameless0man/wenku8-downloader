...
class Scheduler {
    ...
    err() {
        ...
        this.wait *= 2;
        this.wait = Math.min(this.wait, 1000 * 30)
    }
    succ() {
    ...
    status() {
        return `线程数：${this.count} ，等待间隔（秒）： ${this.wait / 1000} ，等待任务数：${this.queue.length} ，运行数： ${this.running}`
    }
}

let scheduler = new Scheduler(30);
...
class Novel {
    ...
    static async rawGet(url) {
        return (await axios.get(url, { responseType: "arraybuffer", timeout: 5000 })).data;
    }
